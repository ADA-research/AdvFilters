...
def automain(_run, eps, max_iters, alpha, verbose):
    ...
    # Load DataModule
    dm = OpenMICDataModule(batch_size_test=1, debug=False, 
                           test_hdf="/hpcwork/rwth1754/openmic/openmic_test.hdf5",
                           train_hdf="/hpcwork/rwth1754/openmic/openmic_train.hdf5")
    dm.setup("test")
    ...
    for x, y, mask in loader:
        ...
        flip_distances = model.net(spec.unsqueeze(0))[0] - 0.5
        flip_idx = flip_distances.abs().argmin().item()
        if verbose:
            print(f"DEBUG: Flipping {flip_idx}")
            print(f"DEBUG: Flip distances: {flip_distances}")
        res = run_pgd_single_sample_flip_one_openmic(model.net, 
                                            spec, 
                                            flip_idx,
                                            device=device,
                                            eps=eps, 
